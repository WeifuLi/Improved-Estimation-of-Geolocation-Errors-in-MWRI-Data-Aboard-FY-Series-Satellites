function [HDF,Errors]=ImprovedGER_Limited(DATA,RegionName,Method)
% Latitude 经度
% Latitude 纬度
% ObserveData 亮度温度
% LandSeaMask 海陆掩码
...
switch RegionName
    case '阿拉伯海岸'
        Lat_min=12; Lat_max=24; Lon_min=42; Lon_max=62;
    case '澳大利亚西海岸'
        Lat_min=-39; Lat_max=-20; Lon_min=109; Lon_max=122;
    case '利比亚'
        Lat_min=0; Lat_max=40;Lon_min=15; Lon_max=30;
    otherwise
        error('The RegionName is invalid. Supported regions are: 阿拉伯海岸, 澳大利亚西海岸, 利比亚');
end

%Step 2: 选取T6中每一个非零值对应的FOV及其半径R邻域的FOV，当做穿过海岸线的区域
R=10;
Miss=0;
for i=R+1:1.5*R:size(T6,1)-R
    for j=51:1.5*R:size(T6,2)-50
       ...
       %% Coastline points dtection
        ...
        if Latitude(i,j)<Lat_min||Latitude(i,j)>Lat_max||Longitude(i,j)<Lon_min||Longitude(i,j)>Lon_max
            continue;
        end
        if sum(sum(T))<=1
            continue;
